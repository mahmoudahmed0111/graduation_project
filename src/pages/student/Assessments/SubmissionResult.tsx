...
export function SubmissionResult() {
  ...
  if (detail.isError || !detail.data) {
    return (
      <Card>
        <CardContent className="p-12 text-center text-red-600">{t('student.submissionResult.notFound')}</CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <Link to="/dashboard/assessments/submissions" className="inline-flex items-center gap-1 text-sm text-primary-600">
        <ArrowLeft className="h-4 w-4" /> {t('student.submissionResult.backToSubmissions')}
      </Link>
      ...
      <Card>
            ...
            <span className="text-sm font-normal text-gray-500">
              {s.status === 'graded' ? (
                <span className="flex items-center gap-1 text-green-700">
                  <CheckCircle2 className="h-4 w-4" /> {t('student.submissionResult.graded')}
                </span>
              ) : s.status === 'submitted' ? (
                <span className="flex items-center gap-1 text-blue-700">
                  <Clock className="h-4 w-4" /> {t('student.submissionResult.awaitingGrade')}
                </span>
              ) : (
                <span>{s.status}</span>
              )}
        ...
        <CardContent className="space-y-4">
          ...
          {s.submittedAt && (
            <p className="text-sm text-gray-500">{t('student.submissionResult.submittedAt', { date: new Date(s.submittedAt).toLocaleString() })}</p>
          )}

          <div className="space-y-3 pt-4 border-t border-gray-100">
            <h3 className="font-semibold">{t('student.submissionResult.answers')}</h3>
            {s.answers.map((a, idx) => (
              <div key={a.questionId ?? idx} className="border border-gray-200 rounded p-3">
                ...
                {a.fileUrl && (
                  <a href={a.fileUrl} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline">
                    {t('student.submissionResult.viewFile')}
                  </a>
                )}
                {(a.score != null || a.feedback) && (
                  <div className="mt-2 pt-2 border-t border-gray-100 text-sm space-y-1">
                    {a.score != null && <div className="font-medium">{t('student.submissionResult.score')}: {a.score}</div>}
                    {a.feedback && (
