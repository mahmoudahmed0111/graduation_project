      ...
      <section className="relative bg-gradient-to-br from-primary-900 to-indigo-900 text-white py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative animate-fade-in-up">
          <p className="inline-block px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-accent-300 text-xs font-semibold uppercase tracking-wider mb-4">
            {t('public.termsOfUse.kicker')}
          </p>
          <h1 className="text-4xl md:text-5xl font-bold mb-3">{t('public.termsOfUse.heroTitle')}</h1>
          <p className="text-primary-100 max-w-2xl">
            {t('public.termsOfUse.heroSubtitle')}
          </p>
          <p className="mt-4 text-xs text-primary-200">{t('public.termsOfUse.lastUpdated')}</p>
        </div>
      </section>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-3">{t(`public.termsOfUse.${s.titleKey}`)}</h2>
                      <div className="space-y-3">
                        {s.bodyKeys.map((bk, i) => (
                          <p key={i} className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{t(`public.termsOfUse.${bk}`)}</p>
                        ))}
                      </div>
                    </div>
                  </div>
