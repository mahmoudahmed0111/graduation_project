...
export function EditCourseOfferingPage() {
  ...
  const handleSave = async () => {
    ...
    if (!Number.isFinite(ms) || !Number.isFinite(tdNum)) {
      showError(t('admin.editCourseOfferingPage.numbersOnly'));
      return;
    }
    if (!policyOk) {
      showError(t('admin.editCourseOfferingPage.gradingMustSum'));
      return;
    }
    ...
    try {
      ...
      success(t('admin.editCourseOfferingPage.updatedToast'));
      navigate('/dashboard/academic/offerings');
    } catch (err) {
      showError(getApiErrorMessage(err, t('admin.editCourseOfferingPage.updateFail')));
    }
  };
  ...
  if (isError || !offering) {
    return (
      <Card>
        <CardContent className="p-12 text-center text-red-600">{t('admin.editCourseOfferingPage.notFound')}</CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="w-full max-w-full space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/dashboard/academic/offerings">
          <Button variant="ghost" size="sm">
            <ArrowLeft className="mr-1 h-4 w-4" /> {t('admin.editCourseOfferingPage.back')}
          </Button>
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('admin.editCourseOfferingPage.title')}</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('admin.editCourseOfferingPage.details')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {t('admin.editCourseOfferingPage.fixedHint')}
          </p>

          <div className="grid gap-4 md:grid-cols-2">
            <Input label={t('admin.editCourseOfferingPage.maxSeats')} value={maxSeats} onChange={(e) => setMaxSeats(e.target.value)} />
            <Input label={t('admin.editCourseOfferingPage.totalDegree')} value={totalDegree} onChange={(e) => setTotalDegree(e.target.value)} />
          </div>

          <div>
            <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.editCourseOfferingPage.gradingPolicy')}</p>
            ...
            {!policyOk && (
              <p className="mt-2 text-sm text-amber-700 dark:text-amber-300">
                {t('admin.editCourseOfferingPage.gradingMismatch', { sum: sumGradingPolicy(policy), total: totalDegree })}
              </p>
            )}
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('admin.editCourseOfferingPage.doctorsHint')}
              </p>
            ...
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.editCourseOfferingPage.tas')}</p>
              <div className="max-h-48 space-y-1 overflow-y-auto rounded border border-gray-200 p-2 dark:border-dark-border">
                {taOptions.map((opt) => (
                  <label key={opt.value} className="flex items-center gap-2 text-sm">
                    <input
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.editCourseOfferingPage.schedule')}</p>
              <Button type="button" size="sm" variant="secondary" onClick={addSlot}>
                {t('admin.editCourseOfferingPage.addSlot')}
              </Button>
            </div>
            {schedule.map((slot, i) => (
              <div
              ...
              >
                <Select2
                  label={t('admin.editCourseOfferingPage.day')}
                  options={OFFERING_FORM_DAYS}
                  value={slot.day}
                  onChange={(v) => updateSlot(i, { day: v })}
                  searchable={false}
                />
                <Input label={t('admin.editCourseOfferingPage.start')} value={slot.startTime} onChange={(e) => updateSlot(i, { startTime: e.target.value })} />
                <Input label={t('admin.editCourseOfferingPage.end')} value={slot.endTime} onChange={(e) => updateSlot(i, { endTime: e.target.value })} />
                <Select2
                  label={t('admin.editCourseOfferingPage.location')}
                  options={locationOptions}
                  value={slot.location}
                  onChange={(v) => updateSlot(i, { location: v })}
                />
                <Select2
                  label={t('admin.editCourseOfferingPage.session')}
                ...
                <div className="flex items-end">
                  <Button type="button" size="sm" variant="ghost" onClick={() => removeSlot(i)}>
                    {t('admin.editCourseOfferingPage.remove')}
                  </Button>
                </div>
              </div>
            ))}
          ...
          <div className="flex justify-end gap-2 border-t border-gray-100 pt-4 dark:border-dark-border">
            <Link to="/dashboard/academic/offerings">
              <Button type="button" variant="secondary">
                {t('admin.editCourseOfferingPage.cancel')}
              </Button>
            </Link>
            <Button type="button" variant="primary" onClick={handleSave} disabled={updateMut.isPending || !policyOk}>
              <Save className="mr-2 h-4 w-4" />
              {updateMut.isPending ? t('admin.editCourseOfferingPage.saving') : t('admin.editCourseOfferingPage.save')}
            </Button>
          </div>
        </CardContent>
      </Card>
